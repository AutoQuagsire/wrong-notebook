    ...
    const handleTextSubmit = async (questionText: string) => {
        ...
        try {
            ...
            if (useLocalLlm) {
                ...
                frontendLogger.info('[AddTextSubmit]', 'Using client LLM', { provider: localConfig.provider });

                try {
                    result = await clientReanswerQuestion({ questionText });
                } catch (error: unknown) {
                    console.error("[AddTextSubmit] Client LLM failed:", error instanceof Error ? error.message : String(error));
