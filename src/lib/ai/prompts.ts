...
<question_text>
在此处填写题目的完整文本。使用 Markdown 格式。所有数学公式使用 LaTeX 符号（行内 $...$，块级 $$...$$）。严禁使用 \\(...\\) 和 \\[...\\]。
...
* **公式格式要求**：行内公式统一使用 $...$；块级公式统一使用 $$...$$；**严禁使用 \\(...\\) 和 \\[...\\]**；禁止使用 \\mathcal、\\mathscr 等花体命令；禁止使用 \\left、\\right 修饰符（用 \\big 系列或不带大小修饰符）。
* **结构化输出**：先写结论，再列关键依据（编号列表），然后是必要推导（含公式），最后指出易错点。
* **简洁性**：解析正文控制在 15 行以内；公式只在关键步骤展示，避免堆砌；不使用寒暄或评价性语言。
</analysis>
...
* **公式格式要求**：行内公式统一使用 $...$；块级公式统一使用 $$...$$；**严禁使用 \\(...\\) 和 \\[...\\]**；禁止使用 \\mathcal、\\mathscr 等花体命令。
* **结构化输出**：先写结论，再列关键依据（编号列表），然后是必要推导（含公式），最后指出易错点。
* **简洁性**：解析正文控制在 10 行以内；公式只在关键步骤展示，避免堆砌；不使用寒暄。
</analysis>
...
<answer_text>
在此处填写正确答案。使用 Markdown 和 LaTeX 符号（行内 $...$，块级 $$...$$）。禁止使用 \\(...\\) 和 \\[...\\]。
</answer_text>
...
* **直接使用标准的 LaTeX 符号**（如 $\\frac{1}{2}$），**不要**进行 JSON 转义。
* 解析要清晰、完整，适合学生理解。
* **公式格式要求**：行内公式统一使用 $...$；块级公式统一使用 $$...$$；**严禁使用 \\(...\\) 和 \\[...\\]**；禁止使用 \\mathcal、\\mathscr、\\left、\\right 等复杂命令。
* **结构化输出**：先写结论，再列关键依据（编号列表），然后是必要推导（含公式），最后指出易错点。
* **简洁性**：解析正文控制在 15 行以内；公式只在关键步骤展示；不使用寒暄。
</analysis>
